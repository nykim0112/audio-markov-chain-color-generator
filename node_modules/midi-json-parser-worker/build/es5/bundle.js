(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.workerFactory, global._defineProperty));
    ...
    var _parseEvent = function _parseEvent(dataView, offset, lastStatusByte) {
      ...
      return _objectSpread(_objectSpread({}, result), {}, {
        event: _objectSpread(_objectSpread({}, result.event), {}, {
          delta: delta
        }),
    ...
    var _parseMetaEvent = function _parseMetaEvent(dataView, offset) {
      ...
      if (metaTypeByte === 0x01) {
        ...
        event = {
          setTempo: {
            microsecondsPerQuarter: (dataView.getUint8(nextOffset) << 16) + ( // tslint:disable-line:no-bitwise
        ...
        var hourByte = dataView.getUint8(nextOffset); // tslint:disable-next-line:no-bitwise

        if ((hourByte & 0x60) === 0x00) {
          frameRate = 24; // tslint:disable-next-line:no-bitwise
        } else if ((hourByte & 0x60) === 0x20) {
          frameRate = 25; // tslint:disable-next-line:no-bitwise
        } else if ((hourByte & 0x60) === 0x40) {
          frameRate = 29; // tslint:disable-next-line:no-bitwise
    ...
    var _parseTrackChunk = function _parseTrackChunk(dataView, offset) {
      ...
      while (nextOffset < length) {
        ...
        events.push(event);
        nextOffset = result.offset; // tslint:disable-next-line:no-bitwise
